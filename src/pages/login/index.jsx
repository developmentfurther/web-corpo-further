...
export default function Login() {
  ...
  // 🔹 Detección de admin desde tu perfil actual
  ...
  // 🔹 Si ya hay sesión y es admin → redirigir
  useEffect(() => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // 🔹 Una vez logueado, si es admin, redirigimos
      if (isAdmin) router.replace(ADMIN_PATH);
    } catch (err) {
      console.error(err);
      const code = err?.code || "";
  ...
  // (todo lo que ya tenés a partir del <div className="min-h-screen ...">)
  // no necesita cambiar absolutamente nada.
