...
// ✅ Card optimizada - sin blur, solo opacity y scale
const ServiceCard = memo(({ card, pos, slotToX, styleByPos, trans, visitText }) => (
  <motion.div
    key={card.id}
    initial={{ x: slotToX(pos), opacity: 0, scale: 0.8 }}
    animate={{ x: slotToX(pos), ...styleByPos(pos) }}
    transition={trans}
    className="absolute -translate-x-1/2 -translate-y-1/2"
    style={{ 
      width: 256,
      willChange: 'transform, opacity' // ✅ Hint para GPU acceleration
    }}
  >
...
export default function Services() {
  ...
  const indices = useMemo(() => ({
    left: (centerIndex - 1 + n) % n,
    center: centerIndex,
    right: (centerIndex + 1) % n,
  }), [centerIndex, n]);

  const visible = useMemo(
    () => [
      { card: CARDS[indices.left], pos: -1 },
      { card: CARDS[indices.center], pos: 0 },
      { card: CARDS[indices.right], pos: 1 },
    ],
    [CARDS, indices]
  );

  const goNext = useCallback(() => setCenterIndex((i) => (i + 1) % n), [n]);
  const goPrev = useCallback(() => setCenterIndex((i) => (i - 1 + n) % n), [n]);

  const constants = useMemo(() => ({
    slotToX: (pos) => pos * (256 + 32),
    trans: {
      type: "spring",
      stiffness: 140, // ✅ Más rápido sin blur
      damping: 28,
    },
    // ✅ CLAVE: Usar solo opacity y scale (sin blur ni brightness)
    styleByPos: (pos) => {
      const isCenter = pos === 0;
      return {
        scale: isCenter ? 1.15 : 0.88, // ✅ Más contraste en escala
        opacity: isCenter ? 1 : 0.35,  // ✅ Más transparencia lateral
    ...
    <>
      <section className="relative py-32 overflow-hidden bg-gradient-to-b from-[#0A1628] via-[#0C212D] to-[#0C212D] text-white">
        ...
        <div className="relative max-w-6xl mx-auto px-8">
          ...
          {/* ✅ Añadir transform-gpu para forzar aceleración */}
          <div
            className="relative h-[520px] transform-gpu"
            ...
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
              {visible.map(({ card, pos }) => (
                <ServiceCard
                  ...
                  card={card}
                  pos={pos}
                  slotToX={constants.slotToX}
                  styleByPos={constants.styleByPos}
                  trans={constants.trans}
                  visitText={t("visitButton")}
                />
              ))}
            </div>
          </div>
